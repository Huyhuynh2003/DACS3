...
class NhanVien : AppCompatActivity() {
    ...
    private val urlGetData: String = "http://192.168.1.5/Do_an_cs3/getdataNhanvien.php"
    private val urlAddComment: String = "http://192.168.1.5/Do_an_cs3/addNvien.php"
    private val urlDeleteComment: String =
        "http://192.168.1.5/Do_an_cs3/deleteNvien.php"
    private val urlEditComment: String = "http://192.168.1.5/Do_an_cs3/editNvien.php"

    private var mangComment: ArrayList<Employee> = ArrayList()
    private var adapterComment: ArrayAdapter<Employee>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lvComment.setOnItemClickListener { parent, view, position, id ->
            val selectedEmployee = mangComment[position]
            selectedComment = selectedEmployee.name
            editTextNoidung.setText(selectedComment)
            editTextPhoneNumber.setText(selectedEmployee.phone)
            editTextAddress.setText(selectedEmployee.diachi)
        }
    ...
    inner class GetData : AsyncTask<String, Void, String>() {
        ...
        override fun onPostExecute(result: String?) {
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                val name = objectComment.getString("name")
                val phone = objectComment.getString("phone")
                val diachi = objectComment.getString("diachi")
                val employee = Employee(name, phone, diachi)
                mangComment.add(employee)
            }
    ...
    private fun addComment() {
        ...
        val diachi = editTextAddress.text.toString()
        val selectedEmployee = mangComment.find { it.name == selectedComment }

        if (selectedEmployee != null) {
            val url = "$urlAddComment?name=$comment&phone=$phone&diachi=$diachi"
            AddCommentTask().execute(url)
        } else {
            Toast.makeText(
                applicationContext,
                "Vui lòng chọn một nhân viên để thêm bình luận",
                Toast.LENGTH_SHORT
            ).show()
        }
    ...
    private fun editComment() {
        val name = editTextNoidung.text.toString()
        val phone = editTextPhoneNumber.text.toString()
        val diachi = editTextAddress.text.toString()
        val selectedEmployee = mangComment.find { it.name == selectedComment }

        if (name.isNotEmpty() && phone.isNotEmpty() && diachi.isNotEmpty() && selectedEmployee != null) {
            val encodedSelectedComment = java.net.URLEncoder.encode(selectedComment, "UTF-8")
            val encodedNewComment = java.net.URLEncoder.encode(name, "UTF-8")
            val url = "$urlEditComment?oldName=${selectedEmployee?.name}&newName=$encodedNewComment&phone=$phone&diachi=$diachi"

            EditCommentTask().execute(url)
        } else {
            Toast.makeText(
                applicationContext,
                "Vui lòng nhập đầy đủ thông tin và chọn một nhân viên để sửa đổi",
                Toast.LENGTH_SHORT
            ).show()
        }
    ...
    inner class EditCommentTask : AsyncTask<String, Void, String>() {
        ...
        override fun onPostExecute(result: String?) {
            ...
            if (result == "Bình luận đã được chỉnh sửa thành công") {
                ...
                Toast.makeText(applicationContext, "Không thể chỉnh sửa bình luận", Toast.LENGTH_SHORT)
                    .show()
            }
...
data class Employee(val name: String, val phone: String, val diachi: String) {
    override fun toString(): String {
        return "$name - $phone - $diachi"
    }
}
